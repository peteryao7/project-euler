class p054 {
     ...
     * -----
     * 
     * We can store each poker hand as a 24-bit integer. The leading 4 bits
     * represent the rank, and the rest of them are used to determine each
     * card's rank and suit. The one with the larger integer will be deemed
     * the winner. 
     */
    public static class Card {
        ...
        public Card(String s) {
            this("23456789TJQKA".indexOf(s.charAt(0)), 
                "SHCD".indexOf(s.charAt(1)));
        }
    ...
    public static int getNumWins() {
        ...
        for (String hand : HANDS) {
            ...
            for (int i = 0; i < 5; i++) {
                p1[i] = new Card(cards[i]);
                p2[i] = new Card(cards[i + 5]);
            }

            // turn each hand into a 24-bit integer for score
    ...
    public static void main(String[] args) {
        System.out.println(getNumWins());
    }

    private static int getScore(Card[] hand) {
        int[] rankFreq = new int[13]; // rankFreq[i] is the # cards w/ rank i
        int flushSuit = hand[0].suit; // in the range [0,3]
        ...
        // rankFreqHist = # times a rank count of i occurs
        // ex. if there's exactly 1 triplet, then rankFreqHist[3] = 1
        int[] rankFreqHist = new int[6];
        for (int f : rankFreq)
            rankFreqHist[f]++;

        int bestCards = getMostFreqHighestCards(rankFreq, rankFreqHist);
        int straightRank = getStraightRank(rankFreq);

        if (straightRank != -1 && flushSuit == 3)
            return 9 << 20 | straightRank; // royal flush
        else if (straightRank != -1 && flushSuit != -1)
            return 8 << 20 | straightRank; // straight flush
        else if (rankFreqHist[4] == 1)
            return 7 << 20 | bestCards; // 4 of a kind
        else if (rankFreqHist[3] == 1 && rankFreqHist[2] == 1)
            return 6 << 20 | bestCards; // full house
        else if (flushSuit != -1)
            return 5 << 20 | bestCards; // flush
        else if (straightRank != -1)
            return 4 << 20 | straightRank; // straight
        else if (rankFreqHist[3] == 1)
            return 3 << 20 | bestCards; // 3 of a kind
        else if (rankFreqHist[2] == 2)
            return 2 << 20 | bestCards; // 2 pairs
        else if (rankFreqHist[2] == 1)
            return 1 << 20 | bestCards; // 1 pair
        else
            return 0 << 20 | bestCards; // highest card
    }

    private static int getMostFreqHighestCards(int[] ranks, int[] ranksHist) {
        int res = 0;
        int count = 0;
    ...
    private static int getStraightRank(int[] ranks) {
        // breaks out of inner loop only
        outer_loop:
