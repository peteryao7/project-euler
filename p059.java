...
class p059 {
     ...
     * -----
     * 
     * We're given that the key only has 3 lowercase characters, so we can
     * loop over all possible keys and generate the plaintext from the
     * ciphertext using them.
     * 
     * Since we know the plaintext will contain common English characters, we
     * can use a score system to determine whether the plaintext is in English.
     * To do this, we can use a counter that increments every time we get an
     * uppercase or lowercase English character, and decrement otherwise. We
     * want to keep track of the plaintext and key that gives the highest
     * score.
     */
    public static int sumDecrypt(byte[] ciphertext) {
        byte[] bestKey = null; // best 3 bytes (chars) for the key
        byte[] bestPlaintext = null; // best plaintext to be considered correct
        ...
        int sum = 0;
        for (int a = 0; a < bestPlaintext.length; a++)
            sum += bestPlaintext[a];
        
        return sum;
    }

    // scores the deciphered text, returns an int
    // the higher the score the more likely it's right
    private static int score(byte[] b) {
        ...
        for (int i = 0; i < b.length; i++) {
            ...
            if (('A' <= c && c <= 'Z') || ('a' <= c && c <= 'z'))
                ...
                sum--; // don't bail, string could have punctuation, spaces
        }
    ...
    private static byte[] decrypt(byte[] ciphertext, byte[] key) {
        ...
        // XOR each character w/ the generated key
        // % cycles through the key
    ...
    public static void main(String[] args) {
        System.out.println(sumDecrypt(CIPHERTEXT));
    }
